...
public class LTIEntryPointController {
  ...
  @Value("${opendashboard.allowOverrideString}")
  ...
  public String lti(HttpServletRequest request) throws ProviderException, ProviderDataConfigurationException {
    ...
    // set an impersonate user id
    
    String sId = null;
    String sCtx = null;
    
    // allow impersonation if the return URL matches a pattern from the yaml configuration
    Boolean qa = launchRequest.getLaunch_presentation_return_url().contains(this.overrideString);

    if(qa){
	    try{
		    sId = launchRequest.getCustom().get("custom_iuserid");
		    if (sId != null && !sId.isEmpty()){
		    	launchRequest.setUser_id(sId);
		    }
	    } catch(Exception e){
	        logger.debug("No custom user {}", e.getMessage());
	    }
	    try{
		    // set a course id
		    sCtx = launchRequest.getCustom().get("custom_icontextid");
		    if (sCtx != null && !sCtx.isEmpty()){
		    	launchRequest.setContext_id(sCtx);
		    }
	    } catch(Exception e){
	        logger.debug("No custom context {}", e.getMessage());
	    }
    }
    ...
    User u = userProvider.getUserBySourcedId(userProviderData, userSourcedId);
    String ro = u.getRole().toString();
    
    
    // Create a token using spring provided class : LTIAuthenticationToken
    String role = null;
    if (ro.equalsIgnoreCase("teacher")) {
      role = "ROLE_INSTRUCTOR";
    } else if (ro.equalsIgnoreCase("student")) {  
        role = "ROLE_STUDENT"; 
    } else {
    	role = "ROLE_GUEST";
        logger.debug("Setting Guest User: ", launchRequest.getUser_id());
    } 
    ...
    if (StringUtils.isNotBlank(classSourcedId) && role == "ROLE_INSTRUCTOR") {
      cmUrl = String.format("/direct/museCourse/%s",classSourcedId);
    }
    else if (StringUtils.isNotBlank(classSourcedId) && role == "ROLE_STUDENT") {
        cmUrl = String.format("/direct/museStudent/%s/student/%s",classSourcedId,userSourcedId );
      }
    else {
      cmUrl = "/direct/muse";
    }
  ...
  public static boolean hasStudentRole(List<String> studentRoles, String roles){
	 ...
	 for(String studentRole : studentRoles){
