...
public class JdbcClient {
  ...
  public JdbcClient(ProviderData providerData) {
	...
	String connectString = "jdbc:" + this.jdbcConnector.trim()
									+ "://" + this.server.trim()
									+ ":" + this.port.trim()
  ...
  // This method was added as a result of the testing with multiple JDBC providers
  // previously the Rs.isClosed() method was used to check if the Rs 
  // was open prior to closing, this has been replaced with an exception block
  // generally it should not be the case that the Rs is not open when this call 
  // is made, there is no action, other than to record the error and continue
  // (this change was made because of incompatibility with older jdbc drivers)
  ...
  public ResultSet executeSQL(String SQL, Object... sqlparam) {
	  // Get data really only executes a SQL statement, use it as an
	  // overloaded function
	  
	  return this.getData(SQL, sqlparam); 
  }
